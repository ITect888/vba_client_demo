 ...
 *
 */
public class Function2000270Test {
	
	private static Log log = LogFactory.getLog(Function2000270Test.class);
	
	static final String FUNC_NO="2000270";
	 ...
	 * 
	 * 加签说明：对参数（accountType,acctno,biztyp,biztypdesc,certseq,phoneno,placeid,ptyacct,ptycd,sourcechnl,versionno,key(会话密钥)）进行 SHA512签名，其中 key前面的参数按照字母排序，key放置在最后。
	...
	private Map<String, String> buildParams() {
		...
		params.put(FunctionCommon.TO_SIGN_HEAD+"versionno", versionno);
	...
	private void processResult(String result) {
		 Json270 json=JSON.parseObject(result,Json270.class);
		 ...
		 if("0".equals(json.getError_no())) {//系统级调用成功
			 ...
			 Result270 re=json.getResults().get(0);
