 ...
 *
 */
public class Function2000348Test {
	
private static Log log = LogFactory.getLog(Function2000348Test.class);
	
//
static final String FUNC_NO="2000348";
	 ...
	 * 
	 * 
	 * 上送参数（biztyp,biztypdesc,id，placeid,ptyacct,ptycd,sourcechnl,timestamp,key(会话密钥)），其中key前面的是按照字母排序的，key则是要最后附加上去。其中在签名的时候身份证号需要利用会话密钥进行AES加密。
	 * 生成的防篡改签名sign在接口调用时和业务参数一起上传。"
	 * 
	 * 调用2000209实名认证接口：上送参数（biztyp,biztypdesc,id,placeid,ptyacct,ptycd,sourcechnl,timestamp, videopic, usernm,funcNo,sign(签名)）
	...
	private Map<String, String> buildParams() {
		...
		params.put(FunctionCommon.TO_AES_TO_URL_TO_BASE64_HEAD+"id", id);
